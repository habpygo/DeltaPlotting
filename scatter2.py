...
samen = pd.concat([Combined_Dag,Combined_Tijd,Value], axis=1)
samen.columns = ['Dag','Tijd','Value']
#samen.plot(kind='scatter', colormap='rainbow', x='Dag', y='Tijd', c='Value', s=20)

ax = pd_minArray.plot(kind='scatter', x='Dag', y='Tijd', color='Red', label='Low values')
pd_maxArray.plot(kind='scatter', x='Dag', y='Tijd', color='Green', label='Hi values', ax=ax)
